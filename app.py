...
# Otpless sessions MUST survive across requests/workers on Render.
# In-memory alone fails when send hits worker A and verify hits worker B.
_otp_sessions = {}  # phone -> Otpless session dict (hot cache)
_OTP_FILE = os.path.join(STATE_DIR, "otp_sessions.json")


def _otp_load():
    global _otp_sessions
    try:
        if os.path.exists(_OTP_FILE):
            with open(_OTP_FILE) as f:
                data = json.load(f) or {}
            if isinstance(data, dict):
                _otp_sessions.update(data)
    except Exception:
        pass
...
def _otp_save():
    try:
        ...
        tmp = _OTP_FILE + ".tmp"
        with open(tmp, "w") as f:
            json.dump(_otp_sessions, f, ensure_ascii=False)
...
async def meesho_verify_otp(phone_number: str, otp: str, request_id: str, instance_id: str):
    ...
    phone = str(phone_number)[-10:]
    _otp_load()  # pick up session written by another worker / earlier request
...
PLANS = {
    ...
    "pro":     {"key": "pro",     "label": "Pro",       "price": "₹299/mo", "devices": 3, "orders": 60, "color": "#059669", "blurb": "For daily shoppers. 3 accounts · 60 orders/day · priority auto-cancel."},
    "proplus": {"key": "proplus", "label": "Pro Plus",  "price": "₹699/mo", "devices": 0, "orders": 0,  "color": "#047857", "blurb": "For resellers. Unlimited accounts & orders · everything unlocked."},
}


def _secret():
    """Stable signing key. Prefer SESSION_SECRET env (required on Render/ephemeral
    hosts). Fall back to state/secret.key, then a deterministic host-based seed so
    tokens do not flip on every cold start when the disk is wiped."""
    ...
    try:
        ...
        pass
    # Last-resort stable seed so free-tier restarts do not invalidate every token.
    # Users should still set SESSION_SECRET in production.
    import hashlib
    host = (os.getenv("RENDER_EXTERNAL_URL") or os.getenv("WEBAPP_URL") or "meesho-local").strip()
    s = hashlib.sha256(("meesho-stable-" + host).encode()).hexdigest()
    try:
        os.makedirs(STATE_DIR, exist_ok=True)
...
SECRET = _secret()  # signed-token key
...
_PUBLIC_PATHS = ("/", "/api", "/api/bootstrap", "/api/auth/login", "/api/auth/me",
                 ...
                 "/api/auth/json_login", "/api/saas/plans", "/api/auth/import_account")
_PUBLIC_PREFIXES = ("/api/search", "/api/product", "/api/price", "/api/variation",
                    "/api/suggest", "/api/geocode", "/api/offer", "/api/auth/")
...
# Flat layout: all static assets live next to app.py (no public/ folder).
# Serve common static files from the project root.
...
async def serve_tg():
    return FileResponse(os.path.join(_ROOT, "telegram-web-app.js"), media_type="application/javascript")

@app.get("/leaflet.js")
async def serve_leaflet_js():
    return FileResponse(os.path.join(_ROOT, "leaflet.js"), media_type="application/javascript")

@app.get("/leaflet.css")
async def serve_leaflet_css():
    return FileResponse(os.path.join(_ROOT, "leaflet.css"), media_type="text/css")

@app.get("/layers.png")
async def serve_layers():
    return FileResponse(os.path.join(_ROOT, "layers.png"), media_type="image/png")

@app.get("/layers-2x.png")
async def serve_layers2x():
    return FileResponse(os.path.join(_ROOT, "layers-2x.png"), media_type="image/png")

@app.get("/marker-icon.png")
async def serve_marker():
    return FileResponse(os.path.join(_ROOT, "marker-icon.png"), media_type="image/png")
...
async def api_auth_otp_verify(data: dict = None, request: Request = None):
    ...
    if not res.get("xo") or not res.get("user_id"):
        return {"ok": False, "live": True,
                "error": "Meesho login incomplete (no session token). Request OTP again."}
    try:
        did = (request.headers.get("X-Device-ID") or "").strip() or "default"
    except Exception:
        did = "default"
    ...
    if acc:
        ...
        acc = new_acc
    # Mirror into current request db so same-process follow-up calls see it even
    # if namespace resolution races.
    try:
        if db is not ust:
            db.setdefault("accounts", [])
            if not any(str(a.get("mobile")) == phone for a in db["accounts"]):
                db["accounts"].insert(0, copy.deepcopy(acc))
            db["active_id"] = acc.get("id")
    except Exception:
        pass
    _persist()
    # Sanitize account for JSON (no huge cookie blobs if any)
    acc_out = {k: v for k, v in (acc or {}).items() if k != "cookies"}
    return {"ok": True, "live": res.get("live", False), "token": token,
            "account": acc_out, "accounts": [acc_out], "active_id": acc_out.get("id"),
            **_auth_me(user)}
...
async def _real_cart_review(address_id=None, dest_pin=None):
    """Load the REAL cart for the active saved account via api/9.0/cart.
    Optional address_id binds delivery in the same call (avoids a separate
    location call that rotates the session and triggers cart-updated rejects).
    ...
    body = {
        ...
        "dest_pin": dest_pin, "address_id": int(address_id) if address_id else None,
    ...
    return {
        ...
        "price_break_up": [
            {
                "display_name": (r.get("display_name") or r.get("type") or r.get("label")
                                 or r.get("name") or r.get("title") or ""),
                "type": (r.get("type") or r.get("display_name") or r.get("label") or ""),
                "label": (r.get("label") or r.get("display_name") or r.get("type") or ""),
                "value": r.get("value"),
                "details": [
                    {
                        "display_name": (dt.get("display_name") or dt.get("type") or dt.get("label") or ""),
                        "type": dt.get("type") or "",
                        "label": dt.get("label") or "",
                        "value": dt.get("value"),
                    }
                    for dt in (r.get("details") or [])
                    if isinstance(dt, dict)
                ] if isinstance(r.get("details"), list) else r.get("details"),
...
async def _fresh_checkout_state(need_paymentinfo=True):
    """One-shot checkout state for preorders.

    Strategy (anti cart-updated):
      1. Resolve a valid address_id owned by this Meesho account
      2. Single cart REVIEW with address_id in the same body
      3. Use that response's cart_session + effective_total immediately
      4. Never call paymentinfo / location as separate steps here
    """
    # Resolve address — MUST be a real Meesho address for this account.
    # Ignore local/demo address ids (e.g. 101) that cause preorders to reject.
    try:
        meesho_addrs = await _fetch_meesho_addresses()
    except Exception:
        meesho_addrs = []
    if not meesho_addrs:
        meesho_addrs = _account_addresses() or []
    if not meesho_addrs:
        _dump_preorders("fresh_checkout_no_addr")
        return None

    cart_addr = (db.get("cart") or {}).get("address") or {}
    cart_aid = cart_addr.get("id") or cart_addr.get("address_id")
    addr = None
    if cart_aid is not None:
        for a in meesho_addrs:
            if str(a.get("id") or a.get("address_id")) == str(cart_aid):
                addr = a
                break
    if not addr:
        addr = meesho_addrs[0]
    aid = addr.get("id") or addr.get("address_id")
    if not aid:
        _dump_preorders("fresh_checkout_no_addr_id", addr=addr)
        return None

    review = await _real_cart_review(address_id=aid, dest_pin=addr.get("pin"))
    if not review or not review.get("cart_session"):
        # Fallback: bind then review (legacy path)
        base = await _real_cart_review()
        if not base or not base.get("cart_session"):
            _dump_preorders("fresh_checkout_review_fail")
            return None
        bound_cs, bound_result = await _bind_address_to_cart(base["cart_session"], aid, addr.get("pin"))
        if not bound_cs:
            _dump_preorders("fresh_checkout_bind_fail", addr=addr)
            return None
        db["cart"]["cart_session"] = bound_cs
        review = await _real_cart_review()
        if not review or not review.get("cart_session"):
            _dump_preorders("fresh_checkout_post_bind_fail")
            return None

    cs = review["cart_session"]
    db["cart"]["cart_session"] = cs
    if review.get("address") and review["address"].get("id"):
        addr = review["address"]
        db["cart"]["address"] = addr

    try:
        order_total = float(review.get("effective_total") or 0)
    except Exception:
        order_total = 0.0
    if order_total <= 0:
        _dump_preorders("fresh_checkout_zero_amt", review=review)
        return None

    # Prefer exact total Meesho returned; only round when whole rupees
    if abs(order_total - round(order_total)) < 0.001:
        amt = int(round(order_total))
    else:
        amt = round(order_total, 2)

    upi_amount = None
    for key in ("effective_total_for_upi_plugin", "effective_online"):
        raw = review.get(key)
        if raw is None:
            continue
        try:
    ...
    return {
        "cs": cs, "addr": addr, "amt": amt,
        "order_total": order_total, "upi_amount": upi_amount,
        "items": review.get("items") or [],
        "total_quantity": review.get("total_quantity"),
        "effective_total": order_total, "fod": review.get("fod"),
    }



async def _send_preorders(body, headers, uid):
    """POST /api/4.0/preorders with automatic self-heal retries.

    On ANY failure that looks recoverable (cart shifted, total changed, empty
    session, transient 5xx), rebuild checkout state and retry up to 3 times.
    Returns (ok, d, final_cs, status, final_amt).
    """
    async def _send(cs, amt, address_id):
        b = dict(body)
        b["cart_session"] = cs
        ...
        if address_id is not None:
            try:
                b["address_id"] = int(address_id)
            except Exception:
                pass
        _dump_preorders("preorders_request", body=b)
        return await meesho_request(
            "POST", "https://prod.meeshoapi.com/api/4.0/preorders",
            json=b, headers=headers, timeout=35,
        )
    ...
    for attempt in (1, 2, 3):
        if attempt == 1:
            cs = body.get("cart_session")
            amt = body.get("customer_amount")
            address_id = body.get("address_id")
        else:
            # Full rebuild every retry
            fresh = await _fresh_checkout_state(need_paymentinfo=False)
            if not fresh:
            ...
            address_id = None
            try:
                address_id = int((fresh.get("addr") or {}).get("id") or (fresh.get("addr") or {}).get("address_id"))
            except Exception:
                address_id = body.get("address_id")
            ...
            # brief pause so Meesho settles session
            try:
                import asyncio as _aio
                await _aio.sleep(0.35 * attempt)
            except Exception:
                pass

        resp = await _send(cs, amt, address_id)
        ...
        _dump_preorders("preorders_response", status=last_status, attempt=attempt)
        try:
            d = resp.json()
        except Exception:
            d = {}
        _dump_preorders("preorders_json", d=d, attempt=attempt)

        if isinstance(d, dict) and d.get("order_num"):
            return True, d, final_cs, last_status, final_amt
        ...
        if isinstance(e, dict):
            ...
            msg = str(e.get("message") or e.get("title") or "").lower()
        elif isinstance(e, str):
            msg = e.lower()

        recoverable = (
            (code and code.upper() in _CART_CHANGED_CODES)
            or ("cart has been updated" in msg)
            or ("review your cart" in msg)
            or ("total" in msg and "chang" in msg)
            or (last_status in (429, 500, 502, 503, 504))
            or (not d)
        )
        if attempt < 3 and recoverable:
            _dump_preorders("preorders_retry", code=code, msg=msg, attempt=attempt)
            continue
        return False, d, final_cs, last_status, final_amt
...
def _cart_recompute(force_items=None):
    ...
    db["cart"].update({
        ...
        "fod": fod,
        "price_break_up": [
            {"display_name": "Total MRP", "type": "Total MRP", "label": "Total MRP", "value": round(total_mrp, 2)},
            {"display_name": "Product discount", "type": "Product discount", "label": "Product discount",
             "value": -round(total_mrp - total_pay, 2) if total_mrp > total_pay else 0},
            {"display_name": "Shipping", "type": "Shipping", "label": "Shipping", "value": 0},
            {"display_name": "Total payable", "type": "Total payable", "label": "Total payable",
             "value": round(final, 2), "grand": True},
        ],
    })
    return db["cart"]
...
async def api_order_prices(data: dict = None):
    """Return COD vs Online amounts WITHOUT calling paymentinfo.
    paymentinfo marks the Meesho cart 'updated' and causes preorders to reject
    with CART_INELIGIBLE. Use cart-review totals only.
    """
    data = data or {}
    addr = db["cart"]["address"] or _account_address_by_id() or None
    if not data.get("no_live"):
        # Do NOT call cart/add here — it mutates Meesho cart and causes
        # "cart has been updated" on the next place-order. Review only.
        review = await _real_cart_review()
        if review:
            cs = review.get("cart_session") or db["cart"].get("cart_session") or ""
            if cs:
                db["cart"]["cart_session"] = cs
            # Prefer live review totals; NEVER call paymentinfo here.
            cod = float(review.get("effective_total") or 0)
            online = None
            for key in ("effective_total_for_upi_plugin", "effective_online",
                        "effective_total_with_ppd"):
                raw = review.get(key)
                if raw is None:
                    continue
                try:
                    ...
                    continue
                if v >= 0 and (cod <= 0 or v < cod):
                    online = v
                    break
            if online is None:
                # No invented discount — same as COD until preorders returns real UPI total
                online = cod
            # Keep local cart in sync so UI Order total == breakup
            db["cart"]["effective_total"] = cod
            db["cart"]["effective_online"] = online
            if review.get("price_break_up"):
                db["cart"]["price_break_up"] = review["price_break_up"]
            if review.get("items"):
                db["cart"]["items"] = review["items"]
            fod = review.get("fod")
            return {
                "cod": cod, "online": online,
                "address": review.get("address") or addr,
                "fod": fod, "total": cod,
                "total_mrp": float(review.get("total_mrp") or 0),
                "fod_saved": 0.0 if not fod else max(0.0, cod - float((fod or {}).get("final_price") or cod)),
                "price_break_up": review.get("price_break_up") or db["cart"].get("price_break_up") or [],
                "cart_session": cs,
            }
    tot = float(db["cart"].get("effective_total") or 0)
    online = float(db["cart"].get("effective_online") or tot)
    return {
        "cod": tot, "online": online, "address": addr,
        "fod": db["cart"].get("fod"), "total": tot,
        "total_mrp": db["cart"].get("total_mrp") or 0,
        "fod_saved": db["cart"].get("fod_saved") or 0,
        "price_break_up": db["cart"].get("price_break_up") or [],
    }
...
async def api_place_cod(data: dict = None):
    ...
    if not acc:
        return {"ok": False, "live": True, "error": "no_account",
                "message": "No Meesho account linked. Login with OTP again."}
    if not (acc.get("xo") and acc.get("user_id") and acc.get("instance_id")):
        return {"ok": False, "live": True, "error": "no_session",
                "message": "Meesho session missing (xo). Logout and login with OTP again."}
    h = _active_headers()
    if not h:
        return {"ok": False, "live": True, "error": "no_session",
                "message": "Meesho session expired or invalid. Login with OTP again."}
    uid = acc["user_id"]
...
async def api_order_pay_online(data: dict = None):
    ...
    if not local_items:
        return {"ok": False, "live": True, "error": "empty_cart",
                "message": "Your cart is empty."}
    # Ensure local items exist on Meesho ONLY if live cart is empty.
    # Do not clear+rebuild every time — that dirties the session and triggers
    # "cart has been updated" on preorders.
    existing = await _real_cart_review()
    live_count = len((existing or {}).get("items") or [])
    valid_items = [li for li in local_items
                   if li.get("product_id") and li.get("supplier_id") and li.get("variation_id")]
    if not valid_items and live_count == 0:
        return {"ok": False, "live": True, "error": "empty_cart",
                "message": "No complete items in your cart."}
    if live_count == 0 and valid_items:
    ...
    # Atomic: review -> bind -> re-review -> preorders (no paymentinfo)
    st = await _fresh_checkout_state(need_paymentinfo=False)
    if not st:
        return {"ok": False, "live": True, "error": "cart_review_failed",
                "message": "Could not load the live Meesho cart."}
...
async def api_get_orders(status: Optional[int] = 0, cursor: Optional[str] = None):
    ...
    if live:
        items = list(live.get("orders") or [])
        # Merge very recent local orders so COD/UPI just placed still show even if
        # Meesho list API lags a few seconds.
        seen = {str(o.get("order_num") or o.get("order_id") or "") for o in items}
        for lo in (db.get("orders") or [])[:8]:
            on = str(lo.get("order_num") or "")
            if not on or on in seen:
                continue
            age = int(time.time()) - int(lo.get("created_at") or 0)
            if age > 3600:
                continue
            items.insert(0, {
                "order_num": on,
                "order_id": int(on) if on.isdigit() else None,
                "sub_order_num": lo.get("sub_order_num") or (on + "_1"),
                "status_id": lo.get("status_id") or "STATUS_ID_ORDERED",
                "status_text": lo.get("status_text") or "Order Placed",
                "status_color": lo.get("status_color") or "#038D63",
                "name": lo.get("name"),
                "image": lo.get("image"),
                "quantity": lo.get("quantity") or 1,
                "amount": lo.get("amount"),
                "payment_mode": lo.get("payment_mode"),
                "live": False,
                "local": True,
            })
            seen.add(on)
        if status:
            ...
            for o in items:
                ...
                elif status == 0: filtered.append(o)
            items = filtered if status else items
